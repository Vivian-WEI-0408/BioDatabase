...
api_url = os.environ.get("WebDatabase_URL") or os.environ.get("BIOAPP_API_URL")
if not api_url:
    raise RuntimeError("TFPlot 配置错误：请在 TFPlot/.env 中设置 WebDatabase_URL 或 BIOAPP_API_URL")
...
DimerNameList = list(LBDDimerMenu.keys())
# NRNameList = GetNameList("LBDNR",DataAccess.GetCursor())
DBDNameList = list(DBDMenu.keys())
print(DimerNameList)
print(DBDNameList)
