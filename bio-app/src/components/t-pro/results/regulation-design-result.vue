<template>
  <div class="data-table">
    <div class="res-title">Designing Result</div>
    <div class="scroll-box" style="max-height: 600px;">
      <table>
        ...
        <tbody>
          ...
          <tr>
            <td class="label">Promoter sequence</td>
            <td class="align-left">
              <div class="characters">
                <span
                  v-for="(c, j) in sequence.characters"
                  :key="j"
                  v-tooltip="{ content: sequence.charactersTooltipDict[j], html: true }"
                  :style="{ color: c.color }"
                >{{ c.char }}</span>
              </div>
              <div class="regions">
          ...
          <template v-if="targetBase64.length > 1">
            <tr v-for="(v, i) in targetBase64" :key="'chart-' + i">
              <td class="label">Regulation profile TF({{ i + 1 }})</td>
              <td class="align-left">
          ...
          <tr v-else>
            <td class="label">Regulation profile</td>
            <td class="align-left">
...
export default {
  ...
  computed: {
    ...
    sequence() {
      return this.result?.sequence || { characters: [], charactersTooltipDict: {} };
    },
    regions() {
